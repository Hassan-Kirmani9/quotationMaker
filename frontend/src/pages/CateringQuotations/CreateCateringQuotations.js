...
function CreateCateringQuotation() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await post("/catering-quotations", cateringData)
      
      if (response.success) {
        alert('Catering Quotation created successfully!')
        history.push('/app/catering-quotations')
      } else {
        setApiError(response.message || 'Failed to create catering quotation. Please try again.')
      }
    } catch (error) {
      ...
      if (error.response?.data?.message) {
        ...
        setApiError('Failed to create catering quotation. Please check your connection and try again.')
      }
    } finally {
      setLoading(false)
    }
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <PageTitle>Create New Catering Quotation</PageTitle>
      </div>
      ...
      <div className="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 mt-6">
            ...
            >
              {loading ? 'Creating...' : 'Create Catering Quotation'}
            </Button>
          </div>
        </form>
      </div>
...
export default CreateCateringQuotation